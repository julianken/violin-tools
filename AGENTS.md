...
These run in CI via `.github/workflows/ci.yml` on every PR and on push to `main`. The typecheck/lint gate now enforces **strict TypeScript** (`tsconfig.base.json` adds `noUncheckedIndexedAccess`, `exactOptionalPropertyTypes`, `erasableSyntaxOnly` and 6 more strictness flags) and **type-checked ESLint** (`strictTypeChecked` + `stylisticTypeChecked`, `--max-warnings 0` on the leaf lint script); editing a root config (`tsconfig.base.json`, `eslint.config.js`, `.prettierrc.json`, `.prettierignore`) now busts the Turbo cache via `globalDependencies`. Add new commands here as they land — never claim one that isn't wired (the binding rule is **Agent guardrails** → anti-invention above); the current lifecycle phase is `INSTANCE.md` → "Status".
