...
## Status
**Status: in development — the app builds and runs locally; the deploy pipeline + hosting infra are committed but not yet applied/live.** The foundation (S1, #36) landed a Turborepo + pnpm-workspaces monorepo with `apps/web` (React + Vite + TS) and the four CI gates (`pnpm typecheck` / `pnpm lint` / `pnpm test` / `pnpm build`). S12 (#39) committed the hosting Infrastructure-as-Code (`infra/*.tf`: GCS website bucket + Cloud CDN + global external HTTPS LB + Google-managed TLS + SPA-fallback URL map) and the keyless WIF CI/CD deploy workflow (`.github/workflows/deploy.yml`, push-to-`main`). This is the lifecycle fact for *this* repo (consistent with README §Status). The real build/test/run commands live in `AGENTS.md` → "Working in the tree"; agents claim only commands that are actually wired (the tool-agnostic rule is `AGENTS.md` → "Agent guardrails" → anti-invention). The "shipped & live" flip still pends the gated `terraform apply` + the Cloudflare DNS step (Phase C) — until then nothing is deployed.

## Hosting / domain
- **Domain: `strings-solo.com`** — registered on **Cloudflare**, which is both **registrar and authoritative DNS** (zone `c58ba2a0fd727ad08720af2261c34b3e`). Apex-canonical: the site serves on the apex; `www.strings-solo.com` 301-redirects to apex via a GCP URL-map host rule. The Cloudflare apex/`www` records are **DNS-only (gray-cloud)** → the GCP LB IP (load-bearing for Google-managed-cert validation; see `infra/README.md`).
- **Hosting: GCP project `strings-solo-prod`** (number `48440654897`) — GCS website bucket behind Cloud CDN + a global external HTTPS load balancer with a Google-managed cert covering apex + `www`. Terraform state lives in `gs://strings-solo-prod-tfstate`. CI/CD is **keyless** (Workload Identity Federation; no committed service-account key). The IaC and bootstrap narrative live in `infra/` (`infra/README.md`).
