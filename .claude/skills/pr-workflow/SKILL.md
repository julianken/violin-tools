...
Direct push to `main` is blocked by a GitHub **ruleset** that requires **1 fresh approving review from a non-author collaborator** against the current HEAD (dismissed on any new push). In this repo that can only be `@julianken-bot` — the sole reviewer collaborator (the owner authors the PRs and can't self-approve) — so the bot's approval is what unblocks merge. It is not *enforced* as bot-specific; it's bot-only by virtue of the collaborator set. Every change lands via a PR that (1) fills the template's five sections, (2) earns a bot approval on the exact HEAD being merged, and (3) is **squash-merged through Mergify's queue**: once the approval lands, post `@Mergifyio queue` as a standalone comment and Mergify squash-merges. (`.mergify.yml` gates on the bot approval **and** the `gates` CI check — typecheck/lint/test/build from `.github/workflows/ci.yml`; the branch ruleset requires the same `gates` status check (non-strict) — and the queue is inert until the Mergify GitHub App is granted access to the repo.)
...
4. For a UI PR, attach the screenshots to the PR description **first** (see "Screenshots — never committed" below) — the reviewer must inspect the published images at the approved HEAD, so they have to exist before the review runs. Then dispatch a fresh-context reviewer on the anti-slop rubric (`.claude/skills/reviewing/SKILL.md`); to post the gating `@julianken-bot` verdict, load the user-level `reviewing-as-julianken-bot` overlay for the credentials/REST-API mechanics. Do NOT call `gh pr review` yourself (rule 2).
...
**Attach screenshots BEFORE the review is dispatched.** The screenshots must be in the PR description *before* you dispatch the reviewer, so the reviewer inspects the actual published images at the approved HEAD — not a promise of images to come later. The reviewing rubric (`.claude/skills/reviewing/SKILL.md` R12) requires the reviewer to fetch and visually inspect each attached image; attaching them after the review runs means the correctness reviewer never sees them and a stale/broken/wrong-state screenshot can pass unseen. If a review-driven fix changes the UI, **re-capture → re-attach → re-review** on the new HEAD. A body-only screenshot edit (swapping in a corrected image with no code change) does **not** dismiss the per-HEAD approval (rule 3), but a UI code fix does — push it, re-attach the new captures, and re-dispatch the bot.
