...
5. **Plan / issue reference** — link the tracker issue this PR implements (the epic or child issue; plans live in the tracker, not a committed `docs/plans/` file), or write `Out of plan — <one-line reason>`; never leave it blank. Include `Closes #<issue>` so the merge auto-closes the issue. This is the bidirectional traceability link the reviewer uses to confirm the PR implements exactly the planned task — no scope creep, no skipped prereqs.
