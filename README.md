...
The four CI gates are `pnpm typecheck` / `pnpm lint` / `pnpm test:coverage` (the Test gate, which runs the Vitest suites and enforces per-workspace coverage thresholds) / `pnpm build`; a separate soft Playwright suite (`pnpm test:e2e`) covers motion, accessibility, mobile reflow, and the v1 acceptance + visual flows. See [`AGENTS.md`](./AGENTS.md) → "Working in the tree" for the full command set.
