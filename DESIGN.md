...
- **Discrete fingerboard-window re-frame.** The window is **anchored at the open end** (the nut stays visible) for the whole of first position — the open string through the 4th-finger reach — so a target there never displaces the open column; it advances up the neck only once the active target climbs **beyond** first position, and then by a single discrete re-frame (not a continuous scroll). Because targets are assigned to the highest open string at or below their pitch, a 2-octave scale's notes distribute across the four strings and usually stay in first position, so the window commonly never moves at all. Off-window columns are **clipped** (a real `clipPath` on the translated dot group) — hidden, never half-painted at the board edge. The re-frame fires **one §7 transition** (the shortest sanctioned discrete transition, §7.1 "discrete") on the window's position — instant under `reduce` (the §7.4 `transition: none` guard).
- **String identifiers follow the §12.2 column-0 rule.** The drill map renders a string's name (`G3`/`D4`/`A4`/`E5`) at its open slot **iff no drill-target dot occupies that slot** — occupancy is **plan-level** (does the whole plan put a target at `columnOffset 0` on this string?), never window-visibility, so a name never flashes in/out as the active target moves *within* first position. The name glyphs are fixed chrome, pinned outside the translated window group. The open-end chrome as a whole — the string names, the nut, and the "open" label — is present only while the **open column is on screen** (the window is anchored at the open end); once the window advances into the upper neck there is no open column to label, so it is absent rather than mislabeling the scrolled dots.
...
- **The discrete fingerboard-window re-frame** fires one §7 transition per position jump (the `transition` on the window's scroll/transform). Under `reduce`, the re-frame snaps — no `transition` on the window's position. Guard: `.board[data-motion] .drill-window { transition: none }` under `reduce` (§7.4 pattern).
