...
> **S16 ph3 (mobile model — current).** Below `760px` the 248px sidebar is **hidden**, the mobile **top-bar search trigger** opens the command palette, and the controls collapse to a one-tap **summary bar** that opens a **non-modal bottom sheet** (peek→expand) hosting Root (a 4×3 grid) · Scale · Refs · the **View row** (orientation/density/handedness). The off-canvas drawer was dropped. The per-element table, the breakpoint table, and the bottom-sheet motion paragraph below describe this shipped model. **Still open for S16 Phase 4 (#78):** the **desktop** View row in the controls card — the desktop card does not yet carry the orientation/density/handedness toggles (they live only in the mobile sheet today).

The shell is designed desktop-first and **reflows to a real single-column mobile layout** below one breakpoint (S11; the mobile surface re-authored in S16 ph3). This is no longer a "narrow floor placeholder": below `760px` the 248px sidebar is hidden (the off-canvas **drawer** it used to become was **dropped in S16 ph3**), the content column and note-map plate take the full viewport width, the controls collapse to a summary bar + bottom sheet, and **the page never overflows horizontally** — the headline invariant is `document.scrollingElement.scrollWidth <= clientWidth` at `390px` **and `320px`** (the historical 458px overflow is gone). The desktop layout at and above the breakpoint is unchanged.

**Auto vertical note map on mobile (S16 ph2).** A second, orthogonal axis to the shell reflow above: the note map itself **auto-rotates to its vertical render in portrait** so it fits the viewport width instead of horizontally scrolling a 760px-wide SVG. The breakpoint trigger is `matchMedia('(orientation: landscape)')`, read in `useMapView` (via `useIsLandscape`): portrait (not landscape) → vertical; landscape → horizontal. The CSS that delivers it is `.board[data-orientation='vertical'] { min-width: 0 }` (`apps/web/src/shell/shell.css`) — the vertical viewBox is intrinsically narrow (`0 0 352 850`, §12.1) but would still paint at the `760px` horizontal floor without this override. **AUTO is the default**; **S16 ph3** added the **mobile** manual orientation/density/handedness toggle set (the bottom-sheet **View row** — §16, and the line-873 status note above). The **desktop** View row remains deferred (S16 Phase 4, #78).
...
**Bottom-sheet motion (§7).** The non-modal controls sheet slides peek↔expand as the transitions-dev panel-reveal (07) **technique**, applied **transform-only on `translateY`** — #07's stock opacity-fade and filter-blur are deliberately dropped so the always-visible peek band stays legible (`opacity` stays `1`, no blur), the same trim the in-tree tape slide uses. The transform is driven by `data-open` on `.controls-sheet` (`.mc-sheet` carries it), interpolating `translateY(calc(100% - var(--sheet-peek-h)))` (peek) ↔ `translateY(0)` (expanded), with `transition: transform var(--state-color) var(--ease-standard)` — the §7 `--state-color` (200ms) duration and the standard ease, **no spring** (a bounce on a panel enter reads as jitter; §0's spring/overshoot easings are guarded "never chrome") and **no motion library**. The sheet is **non-modal**: it sits at `z-index:40` (a peer of the removed drawer, below the palette overlay at `z-50`), ships **no full overlay scrim**, and the page behind it stays live — dismissal is the close button + `Esc` + the drag handle (`apps/web/src/controls/MobileControls.tsx` via `useDrawer.ts`). The slide is gated on `prefers-reduced-motion: reduce` (§7.4) — under `reduce` the sheet snaps peek↔expand with no slide (the end-state transforms still apply). The `--sheet-peek-h` (52px) token is §0 (`apps/web/src/controls/controls.css`).
...
- **Mobile reflow** (§10): below `760px` the shell stacks to one full-width column — the 248px sidebar is hidden (the mobile top-bar search trigger + the controls bottom sheet take over its role; the off-canvas **drawer was dropped in S16 ph3**), and the controls collapse to a one-tap **summary bar** that opens a **non-modal bottom sheet** (peek→expand; Root as a 4×3 grid, plus the View row). The note map **renders vertical (auto) on a portrait phone** (§12.1) instead of horizontally scrolling the 760px SVG — no horizontal overflow of the page *or* the plate at `390px` **or `320px`**. _(The full §10 prose now matches this model; only the **desktop** View row remains for S16 Phase 4 — #78.)_
...
- **URL-encoded deep-linking** — the app holds selection in client state only; no shareable per-scale URL yet (the "Share scale" button is present chrome with no defined behavior). The Cloudflare Worker's SPA fallback (§ hosting) is forward-prep for when deep links land.
...
- The **theme toggle's toggled/active behavior** is unspecified (§8.8): its resting + hover visuals are defined, but with no light mode there is no switch behavior, checked state, or theme swap yet. The non-functional theme toggle and "Share scale" button are present in chrome but have no defined behavior.
...
- **Mobile note-map reflow — AUTO orientation + the MOBILE manual controls ship (S16 ph2/ph3).** The map **auto-rotates** (horizontal on desktop, vertical on a portrait phone, upright labels, density-derived spacing — §10/§12.1) so it fits viewport width instead of horizontally scrolling the 760px SVG. **S16 ph3** then shipped the **mobile** manual view controls: the controls "View" row (the **orientation** Auto/Vertical/Horizontal, **density** Auto/Fit/Comfort, and **handedness** Right/Left segmented toggles, wired to `useMapView` and persisted) now lives in the mobile **bottom sheet** (a one-tap summary bar opens a non-modal peek→expand sheet), and the mobile off-canvas **drawer was dropped** (search moved to the top bar). **Still deferred to a later phase (S16 Phase 4, #78):** the **desktop** View row in the controls card (the desktop card does not yet carry the toggles). _(The §10 prose has been reconciled to the dropped-drawer / bottom-sheet model — no longer pending.)_
- **Reference overlays are horizontal-only; on the vertical map they are disabled, not broken (S16 ph2 — "U3b").** The §12.3 reference overlays (tape bands, the `low 2` slide, the heel/octave landmark bands, position labels) still build their geometry on the horizontal `xOf` column axis — they are **not yet projected through `axisOf`**, so they are axis-correct in the horizontal render only. Re-projecting them so the bands/heel underline/`low 2` slide are axis-correct in **both** orientations is the tracked **U3b** follow-up (#80); until it lands the S16 Phase-2 AC bullet "ref bands/heel/`low 2` are axis-correct in both orientations" is **re-scoped to #80**. The broken state is made **unreachable**, not merely default-off: on the vertical map the Refs pills are disabled (`RefsRow`, `apps/web/src/controls/RefsRow.tsx`) and `<RefLayers>` is skipped entirely (`apps/web/src/notemap/NoteMap.tsx`), so no mis-projected band can paint over the vertical dot grid. On desktop/horizontal the overlays render exactly as before.
