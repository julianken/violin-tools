...
> **S16 ph3 status note (drift — the prose below is mid-rewrite).** S16 Phase 3 **dropped the off-canvas mobile drawer**: below `760px` the 248px sidebar is now hidden, the mobile **top-bar search trigger** opens the command palette, and the controls collapse to a one-tap **summary bar** that opens a **non-modal bottom sheet** (peek→expand) hosting Root (a 4×3 grid) · Scale · Refs · the **View row** (orientation/density/handedness). The per-element table and "Drawer motion" paragraph **below still describe the dropped drawer** — their narrative rewrite is scheduled for **S16 Phase 4** (#78). Until then, **the shipped build is authoritative** (AGENTS.md: shipped build > DESIGN.md > Figma); the no-horizontal-overflow invariant and the auto-vertical map (§12.1) are unaffected and remain accurate.
