...
| Cursor committed config (a `.cursor/rules` pointer, `.cursor/mcp.json`, `.cursor/hooks.json`, `.cursor/commands`) + porting `design-reviewer` to Cursor | Cursor stops reading `AGENTS.md` / `.claude/*` natively (a version regression), OR a built UI / MCP server / CI-style guard need arises | Verified (issue #10): Cursor reads `AGENTS.md` + `.claude/skills` + `.claude/agents` natively (a Cursor 2.4+/2.5+ capability), so committed Cursor config would just be drift — open the repo and it's already governed by `AGENTS.md`. Future caveat: Cursor discovers `design-reviewer.md` but has no per-tool allowlist (degrades to coarse `readonly:true`) and Playwright MCP IDs resolve differently — revisit when a built UI makes the design screenshot pass matter. |
